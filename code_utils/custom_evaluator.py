...
class CustomRerankingEvaluator:
    """
    ...
    def __init__(self, samples, at_k: int = 10, name: str = "", write_csv: bool = True, mrr_at_k: int | None = None):
        ...
        self.csv_file = "CustomRerankingEvaluator" + ("_" + name if name else "") + f"_results_@{self.at_k}.csv"
        self.csv_headers = [
            "epoch",
            "steps",
            f"MRR@{self.at_k}",
            f"NDCG@{self.at_k}",
            f"PRECISION@{self.at_k}",
            f"RECALL@{self.at_k}",
        ]
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        logger.info("CustomRerankingEvaluator: Evaluating the model on " + self.name + " dataset" + out_txt)
        ...
        for instance in self.samples:
            ...
            top_k_true_labels = np.array(is_relevant)[pred_scores_argsort[:self.at_k]]
            all_precision_scores.append(np.mean(top_k_true_labels))
            all_recall_scores.append(np.sum(top_k_true_labels) / np.sum(is_relevant))
        ...
        logger.info(f"PRECISION@{self.at_k}: {mean_precision * 100:.2f}")
        logger.info(f"RECALL@{self.at_k}: {mean_recall * 100:.2f}")
        ...
        if output_path is not None and self.write_csv:
            ...
            with open(csv_path, mode="a" if output_file_exists else "w", encoding="utf-8") as f:
                ...
                writer.writerow([epoch, steps, mean_mrr, mean_ndcg, mean_precision, mean_recall])

        return mean_mrr


def get_hypothesis_dataset(df_test : Any, model : Any, batch_size : int = 64) -> pd.DataFrame:
    """Prepare the test inference scores and relevance along with product/query info.
    Args:
        df_examples_products - test dataset.
        model - fine tuned model.
        batch_size - batch size when processing the dataset.
    Returns:
        df_hypothesis - df with inference scores, relevance, query and product ids.
    """
    ...
    features_query = df_test['query']
    features_product = df_test['product_title']
    ...
    for i in tqdm(range(0, n_examples, batch_size)):
        ...
        scores[i:j] = np.squeeze(model.predict([(q, p) for q, p in zip(features_query_, features_product_)]))
        i = j

    df_hypothesis = pd.DataFrame({
        'query_id' : [str(q) for q in df_test['query_id']],
        'product_id' : df_test['product_id'],
        'scores' : scores,
        'relevance' : [esci_label2relevance_pos[label] for label in df_test['esci_label']]
    })
    df_hypothesis = df_hypothesis.sort_values(by=['query_id', 'scores'], ascending=False)
